...
已移除《义勇军进行曲》的选曲按钮、歌曲配置和两份音频；scene8 清空音频，仅保留场景以维持旧场景索引。当前默认联调入口为 scene9，历史 scene8 测试证据保持原样，不能当作本次曲目变更后的验收。Unity 后端代码由 Qoder CN 生成；图片和动画由即梦 AI 生成。团队确认音乐使用公开、超过版权保护年限的音乐；具体曲目及录音版本的来源依据另行整理。
