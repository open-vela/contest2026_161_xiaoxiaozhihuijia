...
#define PROBE_IMU_CTRL2_G_104HZ_500DPS 0x44
  ...
  if (ret == 0)
    {
      ret = probe_imu_write_verify(fd, PROBE_IMU_ADDR, PROBE_IMU_CTRL2_G,
                                   PROBE_IMU_CTRL2_G_104HZ_500DPS);
    }
          ...
          while (words_to_read > 0)
            {
              ...
              if (pattern_aligned && discard_remaining > 0)
              ...
              else if (pattern_aligned)
                {
                  if (pattern_word < 3)
                    {
                      gyro[pattern_word] = raw;
                      gyro_axis_mask |= 1u << pattern_word;
                      if (pattern_word == 2)
                        {
                    ...
                    {
                      accel[pattern_word - 3] = raw;
                      accel_axis_mask |= 1u << (pattern_word - 3);
                      if (pattern_word == 5)
                        {
                              ...
                              if (gyro_ready)
                                {
                                  ...
                                  sample.monotonic_us = drain_now_us -
                                    (uint64_t)complete_cycles_after *
                                    1000000ULL / PROBE_IMU_RAW_TARGET_HZ;
...
#if BLE_PROBE_GESTURE_TEST_INJECT
      {
...
#else
      printf("openvela_ble_probe: gesture test inject disabled\n");
#endif
      return;
    }
...
int main(int argc, char *argv[])
{
  ...
  if (bt_adapter_register_callback(g_instance, &g_adapter_callbacks) == NULL)
    {
